...
{
    public class GenocideRouteEngine
    {
        ...
        public GenocideRouteEngine(Assembly compiledScripts, string rootPath, IWindowProvider windowProvider)
        {
        ...
        public void Run(string InitialSceneName)
        {
            ...
            try
            {
                TlpLogging.Info("Hello from TestboxedEngine (GenocideRouteEngine because of multithreading).");
                ...
                TestboxedBridge.CameraPosition =
                    new Vector2f(
                        _window.ViewportWidth * 0.5f,
                        _window.ViewportHeight * 0.5f
                    );
                ...
                // Логика отдельно
                Task.Run(() => LogicLoop(_engineCancellation.Token));


                // Аудио отдельно
                Task.Run(() => AudioLoop(_engineCancellation.Token));


                // Главный поток остаётся окном + рендером
                RenderLoop();
            }
            catch (Exception ex)
            {
        ...
        private void LogicLoop(CancellationToken token)
        {
            while (!token.IsCancellationRequested && _window!.IsOpen)
            {
                _window.UpdateDeltaTime();
